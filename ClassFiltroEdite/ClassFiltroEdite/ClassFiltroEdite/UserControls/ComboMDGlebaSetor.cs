...
{
    public partial class ComboMDGlebaSetor : UserControl
    {
       ...
       /* string Neto_Literal = "GLEBA";
        ...
        public ComboMDGlebaSetor()
        {
            ...
            tabAnterior.Columns.Add("SETOR", Type.GetType("System.String"));
            tabAnterior.Columns["SETOR"].MaxLength = 2;
            tabAnterior.Columns.Add("GLEBA", Type.GetType("System.String"));
            tabAnterior.Columns["GLEBA"].MaxLength = 4;
        ...
        public void PegAnterior()
        {
            ...
            try
            {
                dataRow = tabAnterior.AsEnumerable().Where(row =>
                  (row.Field<string>("GLEBA").Trim() == textoNeto.Trim()) &&
                  (row.Field<string>("SETOR").Trim() == textoFilho.Trim())
            ...
            if (dataRow == null)
            {
                ...
                dataRow["ID"] = idMax;
                dataRow["GLEBA"] = textoNeto;
                dataRow["SETOR"] = textoFilho;
        ...
        public bool AtiveString()
        {
            ...
            if (cbNeto.Focused)
            {
                ...
                {
                    string quadra = orow["GLEBA"].ToString();

                    listBoxDialog.Lines.Add(quadra);
                }
                listBoxDialog.Title = "GLEBA";
            ...
            else if (cbFilho.Focused)
            {
                    ...
                    OrderByDescending(row => row.Field<Int32>("ID")))
                {
                    ...
                    string gleba = orow["SETOR"].ToString();
                    listBoxDialog.Lines.Add(gleba);
                }
                listBoxDialog.Title = "SETOR";
                if (listBoxDialog.Execute())
                {
                    // suspendeValidate = true;
       ...
       ref Message msg, Keys keyData)
        ...
        private void ComboNeto_Validated(object sender, EventArgs e)
        {
            ...
            {
                orowQuadras = 
                    (BmNeto.DataSource as DataView).Table.AsEnumerable().Where(row => row.Field<string>("GLEBA").Trim() 
                       == quadra.Trim()).FirstOrDefault();
            }
            catch (Exception)
            {
            }
