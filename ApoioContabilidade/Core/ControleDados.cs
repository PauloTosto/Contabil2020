...
namespace ApoioContabilidade.Core
{
    ...
    static public class AcessosStream
    {
        ...
        static public Dictionary<string, object> LeiaListaInsumos(string filename)
        {
            // Read and show each line from the file. 
            Dictionary<string, object> oresultado = new Dictionary<string, object>();
            string line = "";
            using (StreamReader sr = new StreamReader(filename))
            {
                while ((line = sr.ReadLine()) != null)
                {
                    if (line.Contains("="))
                    {
                        ...
                        string chave = line.Substring(0, pos);
                        string valor = line.Substring(pos + 1);
                        oresultado.Add(chave, (object) valor);
                    }
                }
            }
            return oresultado;
