...
{
    public partial class ComboMDInverte2 : UserControl
    {
        ...
        public ComboMDInverte2()
        {
            ...
            this.cbFilho.GotFocus += CbFilho_GotFocus;
            this.cbFilho.LostFocus += CbFilho_LostFocus;

        }

       

        private void CbFilho_LostFocus(object sender, EventArgs e)
        {
           
          //  (sender as ComboBox).SelectionLength = 0;
        }

        private void CbFilho_GotFocus(object sender, EventArgs e)
        {
            //(sender as ComboBox).SelectionLength = 0;
        }
      ...
      //  private object origvaluePai;
       ...
       /* public ComboBox ComboPai
        ...
        public void ConfigureComboBoxFilhoNeto(Linha olinha,
           BindingSource obmDados, string ofieldNameFilho, string ofieldNameNeto, bool quadraValidaVazio = true)
        {
            fieldNameFilho = ofieldNameFilho;
            fieldNameNeto = ofieldNameNeto;
            Quadra_NOME = fieldNameNeto;
            Gleba_NOME = fieldNameFilho;

            BmFilho = new BindingSource();
            BmFilho.DataSource = DadosComum.GlebasCombo.AsEnumerable().Where(row => row.Field<string>("SETOR").Trim() != ""
                       && row.Field<string>("GLEBA").Trim() != ""
                        
                    ).CopyToDataTable().AsDataView();

            BmNeto = new BindingSource();
            BmNeto.DataSource = DadosComum.QuadrasCombo.AsEnumerable().Where(row => row.Field<string>("SETOR").Trim() != ""
                       && row.Field<string>("GLEBA").Trim() != ""
            ...
            ComboFilho.DisplayMember = "DESCRICAO";
   ...
   */
        private void ComboNeto_Validated(object sender, EventArgs e)
        {
            ...
            try
            {
                orowQuadras = (BmNeto.DataSource as DataView).Table.AsEnumerable().Where(row => row.Field<string>("QUADRA").Trim() == quadra.Trim()).FirstOrDefault();
            }
            catch (Exception)
            {
            }
            if (orowQuadras != null)
            {
                string gleba = orowQuadras["GLEBA"].ToString();
                //string setor = orowQuadras["SETOR"].ToString();
        ...
        private void ComboFilho_Validated(object sender, EventArgs e)
        {
            ...
            try
            {
                orow = (BmFilho.DataSource as DataView).Table.AsEnumerable().Where(row => row.Field<string>("GLEBA").Trim() == gleba.Trim()).FirstOrDefault();
            }
            catch (Exception)
            {
            }
