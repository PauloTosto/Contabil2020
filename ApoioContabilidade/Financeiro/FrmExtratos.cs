...
{
    public partial class FrmExtratos : Form
    {
        ...
        public FrmExtratos(BindingSource obmPagar,
        BindingSource obmReceber)
        {
            ...
            DataTable mov_fin = (bmPagar.DataSource as DataTable).Copy();
            mov_fin.TableName = "MOV_FIN";
            foreach (DataRow orow in (bmReceber.DataSource as DataTable).Rows)
            {
            ...
            try
            {
                ...
                foreach (DataRow orow in mov_fin.Rows)
                {
                    ...
                    if (bcos.Contains(orow["CREDITO"].ToString().Trim()))
                    {
                        ...
                        if (tdeb != "")
                        {
                            oext["FORN"] = orow["DEBITO"].ToString();
                        }
                    }
                    else
                    {
                        ...
                        if (tcre != "")
                        {
                            oext["FORN"] = orow["CREDITO"].ToString();
                        }
                    }
        ...
        private void btnBBrasil_Click(object sender, EventArgs e)
        {
            ...
            /*DataGridViewColumn btnCol = new DataGridViewColumn();
            btnCol.HeaderText = "Concilia";
            btnCol.AutoSizeMode = DataGridViewAutoSizeColumnMode.None;
            btnCol.Width = 45;
            btnCol.DataPropertyName = "CONCILIA";
            ...
            oExtratoBB.oDataGridView.Columns.Insert(0, btnCol);
            */
        ...
        {
         //   if ((extratoBB != null) && (extratoBB.Rows.Count >= 0))
           // {
            //    if (bmExtratoBB.DataSource == null) return;
              //  pesquisaMaisRacional((bmExtratoBB.DataSource as DataView).Table, (bmExtrato.DataSource as DataView).Table);
            //}
        }
