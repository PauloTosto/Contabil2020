...
namespace ApoioContabilidade.Financeiro
{
    public class EdtFin
    {
       ...
       // public System.Windows.Forms.ErrorProvider errorProvider1;
        ...
        const int  col_extra = 1;


        const int DATA_ger = 0 + col_extra;
        const int VALOR_ger = 1 + col_extra;
        const int CONTA_ger = 2 + col_extra;
        const int CONTAFIN_ger = 3 + col_extra;
        const int TITULAR_ger = 4 + col_extra;
        const int HIST_ger = 5 + col_extra;
        const int DOC_ger = 6 + col_extra;
        const int VENC_ger = 8 + col_extra;
        const int DOC_FISC_ger = 7 + col_extra;
        const int OBS_ger = 9 + col_extra;
        ...
        private void MonteEditeReceber(BindingSource bmSource)
        {
            try
            {
               ...
               // bmBancos.DataSource = DadosComum.BancosCombo.Copy().AsDataView();
                ...
                bmDesc2Receber.DataSource = DadosComum.Desc2Combo.Copy().AsDataView();
            ...
            //    BindingSource bmBancos = new BindingSource();
              ...
              //  bmDesc2.DataSource = lstDesc2;
               // bmDesc2.DataSource = DadosComum.Desc2Combo.AsEnumerable().Where(row=>row.Field<String>("DESC2").Substring(0,1) != "*").CopyToDataTable().AsDataView();
                bmDesc2Pagar.DataSource = DadosComum.Desc2Combo.Copy().AsDataView();
