...
{
    class PlanilhaFinanceiraImprime
    {
        ...
        public PlanilhaFinanceiraImprime(DataGridView aDataGridView, PrintDocument aPrintDocument, bool CenterOnPage, bool WithTitle, string aTitleText, Font aTitleFont, Color aTitleColor, bool WithPaging, string aempresatext, string afazendatext)
        {
        ...
        private void DrawHeader(Graphics g)
        {
            ...
            */
            ...
            for (int i = (int)mColumnPoints[mColumnPoint].GetValue(0); i < (int)mColumnPoints[mColumnPoint].GetValue(1); i++)
            {
                ...
                if ((LinhaCab1.Count > 0) && (i < LinhaCab1.Count ))
                {
            ...
            for (int i = (int)mColumnPoints[mColumnPoint].GetValue(0); i < (int)mColumnPoints[mColumnPoint].GetValue(1); i++)
            {
                ...
                if ((LinhaCab2.Count > 0) && (i < LinhaCab2.Count - 1) )
                {
                    // Printing the cell text
                    Font CellFont = TheDataGridView.Columns[i].DefaultCellStyle.Font;
                    if (CellFont != null)
        ...
        private void CalculatePlanilha(Graphics g)
        {
            if (PageNumber == 0) // Just calculate once
            {
              ...
              */
               ...
               // mColumnPointsWidth.Add(mTempWidth);
                mColumnPoint = 0;
                float tamTotalGrid = 0;
                
                for (int i = (int)mColumnPoints[mColumnPoint].GetValue(0); i < (int)mColumnPoints[mColumnPoint].GetValue(1); i++)
                {
                    // Setting the HeaderFore style
                    tamTotalGrid += TheDataGridView.Columns[i].Width;
                }
                ColumnsWidth.Clear();
                for (int i = (int)mColumnPoints[mColumnPoint].GetValue(0); i < (int)mColumnPoints[mColumnPoint].GetValue(1); i++)
                {
                    // Setting the HeaderFore style
                    //if (i < ColumnsWidth.Count)
                       ColumnsWidth.Add((TheDataGridView.Columns[i].Width / tamTotalGrid) * mTempPrintArea);
        ...
        private void DrawTotal(Graphics g)
        {
            ...
            string tituloTotal = "T O T A L";
            if (LinhaTotal.Count > 1)
            {
                tituloTotal = "T O T A I S";
            }
            else if (LinhaTotal.Count == 0)
            { tituloTotal = ""; }
