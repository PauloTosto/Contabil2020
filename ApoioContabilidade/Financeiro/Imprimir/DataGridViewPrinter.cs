...
class DataGridViewPrinter
{
    ...
    private float CurrentY; // A parameter that keep track on the y coordinate of the page, 
    ...
    private bool soColunasVisiveis;
    ...
    public DataGridViewPrinter(DataGridView aDataGridView, PrintDocument aPrintDocument,
        ...
        Font aTitleFont, Color aTitleColor, bool WithPaging,
        bool SoColunasVisiveis = false)
    {
        ...
        soColunasVisiveis = SoColunasVisiveis;
        PageNumber = 0;
    ...
    private void Calculate(Graphics g)
    {
        if (PageNumber == 0) // Just calculate once
        {
            ...
            for (int i = 0; i < TheDataGridView.Columns.Count; i++)
            {
                ...
                if (!soColunasVisiveis ? true : TheDataGridView.Columns[i].Visible)
            ...
            for (k = 0; k < TheDataGridView.Columns.Count; k++)
                if (!soColunasVisiveis ? true : TheDataGridView.Columns[k].Visible)
                {
                    mStartPoint = k;
                    break;
                }
    ...
    private void DrawHeader(Graphics g)
    {
        ...
        {
            if (!(!soColunasVisiveis ? true : TheDataGridView.Columns[i].Visible)) continue; // (!TheDataGridView.Columns[i].Visible) continue; // If the column is not visible then ignore this iteration
    ...
    private bool DrawRows(Graphics g)
    {
        ...
        {
            if (!soColunasVisiveis ? true : (TheDataGridView.Rows[CurrentRow].Visible))    // Print the cells of the CurrentRow only if that row is visible
            {
                ...
                {
                    if (!(!soColunasVisiveis ? true : TheDataGridView.Columns[CurrentCell].Visible)) continue; // If the cell is belong to invisible column, then ignore this iteration
    ...
    public bool DrawDataGridView(PrintPageEventArgs e) // 
    {
        try
        {
            ...
            bool bContinue = DrawRows(e.Graphics);
